...
async def full_analyze(
    ...
    if exif_info and exif_info.latitude and exif_info.longitude:
        try:
            ...
            if geo:
                analysis_result.city = geo["city"]
                analysis_result.country = "대한민국"
                if not analysis_result.confidence or analysis_result.confidence < 0.9:
                    analysis_result.confidence = 0.9
                logger.info(f"역지오코딩 보정: {geo['full_address']} (confidence → 0.9)")
        except Exception as e:
            logger.warning(f"역지오코딩 실패, GPT 결과 사용: {e}")
